...
<body>
        ...
        if($user_email != ""){
           if ($user_role === "customer"){
              ...
              echo "<li role='presentation'><a href='index.php'>Home</a></li>";
              echo "<li role='presentation'><a href='manage_inventory.php'>Products</a></li>";
              echo "<li role='presentation' class='active'><a href='allorders.php'>View All Orders</a></li>";
   ...
   ?>
        ...
        <p> Below are all orders from the store: </p>

<?php
        $statement = $conn->prepare("SELECT O.order_num, O.date_time, O.total, O.status, O.user_email, U.first_name, U.last_name, U.address, U.city, U.zipcode, U.state FROM orders O INNER JOIN users U ON U.email=O.user_email WHERE O.order_num <>(SELECT MAX(O2.order_num) FROM orders O2 WHERE O2.user_email) AND O.date_time ORDER BY O.order_num DESC");
        $statement->execute();
        $statement->bind_result($order_num, $date, $total, $status, $user_email, $first_name, $last_name, $address, $city, $zipcode, $state);
        $order_nums = array();
        $dates = array();
        $totals = array();
        $statuses = array();
        ...
        $statement->close();
        $order_statement = $conn->prepare("SELECT I.name, D.buying_price, D.quantity FROM order_details D INNER JOIN items I on I.prod_id=D.prod_id WHERE D.order_num=?");
        for($i = 0; $i < count($order_nums); $i++){
           $order_statement->bind_param("i", $order_nums[$i]);
           $order_statement->execute();
           $order_statement->bind_result($name, $buying_price, $quantity);
           echo "<div class='panel panel-default'>";
           echo "<div style='height:55px;' class='panel-heading'>";
           echo "<div class='panel-title pull-left'>Order Number: {$order_nums[$i]}</div>";
           echo "<form method = 'post' action = 'shipping.php'>";
           echo "<input class='btn btn-default pull-right' type='submit' value='Ship It'>";
           echo "<input type = 'hidden' value = '{$order_nums[$i]}' name = 'order_num'>";
           echo "</form>";
           echo "</div>";
